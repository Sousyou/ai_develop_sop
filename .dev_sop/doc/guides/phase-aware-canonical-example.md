...
For a copyable artifact, see `.dev_sop/doc/guides/phase-aware-example-main-plan.md`.
...
For a copyable artifact, see `.dev_sop/doc/guides/phase-aware-example-sub-plan.md`.
That example artifact shows:
...
In the clean starter, create the task-layer artifact from `.dev_sop/doc/templates/task-spec-template.md` and keep the parent mapping explicit.
...
In this example, the stable reusable outcome is the long-task example pattern itself, so it is appropriate to update `.dev_sop/doc/facts/golden-cases.md`.
