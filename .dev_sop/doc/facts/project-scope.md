...
Use `.dev_sop/control/CURRENT.md` for live operating state when the control surface is in use.
...
## Stable Phase Goal
Keep the `.dev_sop/control/` surface aligned with the existing spec-first workflow so humans can recover context quickly without turning the repository into a second execution system.
...
- minimal recovery/control surface under `.dev_sop/control/*`
...
- must keep `.dev_sop/control/*` limited to `CURRENT.md` and `DOC_MAP.md`
...
- `.dev_sop/control/*` is the default recovery layer when humans need current-state navigation without colliding with product directories
- durable reusable decision rationale and experiment outcomes should route to facts rather than a standing control-surface ledger
...
- `.dev_sop/doc/facts/project-scope.md` may be mistaken for a live status file instead of a slower-moving scope fact
- `.dev_sop/control/CURRENT.md` may drift into a task log if closeout discipline is not maintained
- `.dev_sop/control/DOC_MAP.md` may expand into a directory catalog instead of staying role-based
