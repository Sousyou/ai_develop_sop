...
Use `.dev_sop/doc/specs/*` to hold narrow task specs that:
...
This starter keeps only template-like example specs in this directory.
Do not treat them as repository work history.
Keep the directory free of starter-owned historical task specs; real projects should add their own task specs here.

## Examples

See these reference specs:
...
- `20260328-003-example-project-control-closeout-loop.md` for a small-task example that also shows the control-surface closeout path

See `.dev_sop/doc/guides/phase-aware-canonical-example.md` for a closed long-task concept chain.
When you need the first real spec, create it from `.dev_sop/doc/templates/task-spec-template.md`.

## Naming

Store each spec as:

`.dev_sop/doc/specs/YYYYMMDD-NNN-task-slug.md`
...
If the starter is still clean, the first same-day sequence is `001`.

Example:

`.dev_sop/doc/specs/20260322-001-auth-session-hardening.md`
...
Use `.dev_sop/doc/templates/release-batch-template.md` if the project wants a lightweight release record.
