...
当 AI 已完成项目入口判定，并建立当前项目的整体认知后，后续执行应遵守“最小必要读取”原则：

1. 不默认在每轮执行前重复全量读取顶层文档。
2. 只读取当前问题直接相关的层级定义、流程文档、模板文档或标准文档。
3. 当前问题若仅涉及 `task` 收口、单步实现或单步验证，不应回到顶层入口链路重新全读。
4. 当前问题若仅涉及事实回写、恢复纠偏或模板落地，只进入对应触发文档，不扩展读取无关层。

### 8.2 何时应重新校准顶层入口

只有当以下情况发生时，才应重新回到顶层入口文档做校准：

1. `project_type` 发生变化。
2. `ai_runtime_sop.md` 中的当前项目运行时补充发生变化。
3. `ai_runtime_standards.md` 中的当前强制标准发生变化。
4. 当前 `phase` 边界、当前主线或项目入口理解被证明已经失效。
