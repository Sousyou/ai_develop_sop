...
1. keep the entrypoints split: root `README.md` for humans, `AGENTS.md` and adapters for AI tools, `.dev_sop/README.md` for the `.dev_sop/*` namespace
...
- keep `.dev_sop/README.md` as the namespace map for AI workflow assets
- keep `.dev_sop/VERSION.md` and `.dev_sop/upgrades/*` so future SOP upgrades can be applied deliberately
- keep project-facing docs outside `.dev_sop/`
- when human readers need a recovery-first current-state view, use `.dev_sop/control/*` and start with `.dev_sop/control/CURRENT.md`
  If you adopt that control surface, also keep `.dev_sop/doc/templates/current-template.md`, `.dev_sop/doc/templates/doc-map-template.md`, and `.dev_sop/skill/session-closeout.md` so it stays small and current.
...
If you want to see one complete long-task chain before copying the pattern, read `.dev_sop/doc/guides/phase-aware-canonical-example.md`.
...
- `.dev_sop/doc/specs/README.md`
- `.dev_sop/doc/facts/facts-index.md`
- `.dev_sop/skill/plan-to-spec.md`
- `.dev_sop/skill/design-whitebox-tests.md`
...
- `.dev_sop/doc/specs/20260104-001-example-first-copied-project-quickstart.md`
- `.dev_sop/doc/guides/phase-aware-workflow.md` when long-task guidance is needed
- `.dev_sop/doc/guides/phase-aware-canonical-example.md`
- `.dev_sop/doc/guides/phase-aware-example-main-plan.md`
- `.dev_sop/doc/guides/phase-aware-example-sub-plan.md`
- `.dev_sop/doc/specs/20260103-001-example-adoption-entrypoint-slice.md`
...
When the work spans multiple milestones, dependencies, or long-lived handoffs, use the phase-aware guidance in `.dev_sop/doc/guides/phase-aware-workflow.md`.
...
Use `.dev_sop/doc/guides/phase-aware-canonical-example.md` when you want a compact, copyable example of that chain.
...
### 3. Derive task specs
Use `.dev_sop/skill/plan-to-spec.md`, `.dev_sop/doc/specs/README.md`, and `.dev_sop/doc/templates/task-spec-template.md`.

A task spec should shrink the plan into a narrow implementation contract.
See `.dev_sop/doc/specs/README.md` for naming, splitting, and lifecycle conventions.
For longer-running work, also use `.dev_sop/doc/guides/task-lifecycle-and-escalation.md` so repair, rollback, replan, and escalation stay explicit.
When a returning human needs to recover the current state before entering the active spec, start with `.dev_sop/control/CURRENT.md`.
...
If you want a copyable default-path example, see `.dev_sop/doc/specs/20260104-001-example-first-copied-project-quickstart.md`.
If you want a copyable example that includes the control-surface closeout path, see `.dev_sop/doc/specs/20260328-003-example-project-control-closeout-loop.md`.
...
See `.dev_sop/doc/guides/testing-strategy.md`.
...
- `.dev_sop/doc/facts/project-scope.md` when scope or boundaries became clearer
- `.dev_sop/doc/facts/golden-cases.md` when stable reusable validation references are worth preserving
...
Use `.dev_sop/doc/templates/release-batch-template.md` if you want a lightweight release record that can be reviewed by both humans and future automation.
...
If design discussion, planning, spec derivation, and execution are split across different tools or different people, use `.dev_sop/doc/guides/design-to-spec-handoff.md`.
