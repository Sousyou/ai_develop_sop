...
For a copyable artifact, see `.dev_sop/core/guides/phase-aware-example-main-plan.md`.
...
For a copyable artifact, see `.dev_sop/core/guides/phase-aware-example-sub-plan.md`.
That example artifact shows:
...
In the clean starter, create the task-layer artifact from `.dev_sop/core/templates/task-spec-template.md` and keep the parent mapping explicit.

That task spec should show:
...
In this example, the stable reusable outcome is the long-task example pattern itself, so it is appropriate to update a project-generated fact file under `.dev_sop/project-facts/*`.
